...
//  class func parseStatus(_ properties: KeyValuesArray, _ inUse: Bool = true) {
  static func parseStatus(_ properties: KeyValuesArray, _ inUse: Bool = true) {
    ...
    if let id =  properties[0].key.streamId {
      ...
      if inUse {
        // YES, does it exist?
        if Objects.sharedInstance.daxRxAudioStreams[id: id] == nil {
          // NO, create a new object & add it to the collection
          Objects.sharedInstance.daxRxAudioStreams[id: id] = DaxRxAudioStream(id)
        }
        // pass the remaining key values for parsing
        Objects.sharedInstance.daxRxAudioStreams[id: id]!.parseProperties(Array(properties.dropFirst(1)) )
        
      } else {
        // NO, does it exist?
        if Objects.sharedInstance.daxRxAudioStreams[id: id] != nil {
          // YES, remove it
          Objects.sharedInstance.daxRxAudioStreams[id: id] = nil
          
//          LogProxy.sharedInstance.log("DaxRxAudioStream removed: id = \(id.hex)", .debug, #function, #file, #line)
          NotificationCenter.default.post(name: logEntryNotification, object: LogEntry("DaxRxAudioStream removed: id = \(id.hex)", .debug, #function, #file, #line))
        }
      }
    }
  }
