...
//    }
    ...
    public func requestMtuLimit(_ size: Int, callback: ReplyHandler? = nil) {
        send("client set enforce_network_mtu=1 network_mtu=\(size)")
    }

    public func requestLowBandwidthDax(_ enable: Bool, callback: ReplyHandler? = nil) {
        send("client set send_reduced_bw_dax=\(enable.as1or0)")
    }
    ...
    public func requestCwKeyImmediate(state: Bool, callback: ReplyHandler? = nil) {
        send("cw key immediate" + " \(state.as1or0)", replyTo: callback)
    }
    ...
    public func requestLicense(callback: ReplyHandler? = nil) {
        send("license refresh", replyTo: callback)
    }
    ...
    public func staticNetParamsReset(callback: ReplyHandler? = nil) {
        send("radio static_net_params" + " reset", replyTo: callback)
    }

    public func staticNetParamsSet(callback: ReplyHandler? = nil) {
        send("radio static_net_params" + " " + StaticNetTokens.ip.rawValue + "=\(staticIp) " + StaticNetTokens.gateway.rawValue + "=\(staticGateway) " + StaticNetTokens.netmask.rawValue + "=\(staticNetmask)")
    }
...
//        send("stream create type=remote_audio_rx compression=\(compression)", replyTo: callback)
//    }
//
...
//        send("stream create type=remote_audio_tx", replyTo: callback)
//    }
//
...
//        send("slice create", replyTo: callback)
//    }
//
...
//            send(cmd, replyTo: callback)
//        }
//    }
//
//    public func requestSlice(panadapter: Panadapter, frequency: Hz = 0, callback: ReplyHandler? = nil) {
//        if availableSlices > 0 {
//            send("slice create " + "pan" + "=\(panadapter.id.hex) \(frequency == 0 ? "" : "freq" + "=\(frequency.hzToMhz)")", replyTo: callback)
//        }
//    }
//
